...
- 시작버튼 > 자격 증면 관리자
  > Windows 자격 증명
  > gti / github / sourcetree 에서 타인의 아이디로 설정 되어 있는 경우 삭제
  > 브라우저에서 github에 로그인 상태 확인
- 사용자 이름과 이메일 설정
  : Windows (Git Bash, com)
  : MacOS ( Git Bash, 터미널 )
  ...
  ? 사용자 이름 설정
  git config --global user.name yua

  ? 사용자 이메일 주소 설정
  git config --global user.email jangyua@naver.com
...
4. remote repository: 인터넷이나 네트위크에 위치한 저장소, GigHub의 호스팅 서비스를 사용

! Git의 주요 명령어
1. 깃 저장소 초기화
>> 명령어 사용 시 해단 폴더에 .git 디렉토리가 생성
: git init
...
git remote add origin https://github.com/yua2003/frontend-study.git
>> 리모트 저장소를 추가

git push -u origin main
>> 로클 리포지토리 main 브랜치를 원격 리포지토리에 푸시
